...
function Crucible(props) {

  const [storedActivity, setStoredActivity] = useLocalStorage('Crucible_Activities', []);
  ...
  const handleRemoveActivity = (id) => {
    setStoredActivity(storedActivity.filter((activity) => activity.id !== id));
  };
  ...
  return (
    <>
      ...
      <div className='crucibleContainer pageContainer'>
        <Button onClick={handleClick} buttonStyle='btn--outline'>Create new Fireteam</Button>
        <h1 className='crucible'>Crucible</h1>
        ...
        <div className={click? 'popup active' : 'popup'}>
          ...
          <form onSubmit={handleOnSubmit}>
            <label>
              <h3>Activity name:</h3>
              <input onChange={handleInputChange} type="text" name="activityname" value={activityname} placeholder='Good luck!'/>
              <h3>Date:</h3>
